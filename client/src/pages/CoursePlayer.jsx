...
const CoursePlayer = () => {
    ...
    const handleLessonComplete = async () => {
        ...
        try {
            ...
            const { progress: newProgress, completedLessons: newCompleted } = res.data.data;
            setProgress(newProgress);
            setCompletedLessons(newCompleted || []);

            if (res.data.alreadyCompleted) {
                toast('Already completed! Moving to next lesson.', { icon: '✓' });
            }

            // Navigate to next lesson if one exists
            const currentIndex = course.lessons.findIndex(l => l._id === activeLesson._id);
            if (currentIndex < course.lessons.length - 1) {
                setActiveLesson(course.lessons[currentIndex + 1]);
                if (!res.data.alreadyCompleted) toast.success('Lesson completed! +100 XP earned.');
            } else {
    ...
    const isFirstLesson = (() => {
        ...
        return course.lessons[0]?._id === activeLesson._id;
    })();
    ...
    return (
        <div className="min-h-screen bg-[#09090b] text-white flex flex-col h-screen overflow-hidden">
            ...
            <div className="flex flex-1 overflow-hidden relative">
                {/* Main Player Area */}
                <main className={`flex-1 overflow-y-auto transition-all duration-500 ${sidebarOpen ? 'lg:mr-80' : 'mr-0'}`}>
                                ...
                                <div className="space-y-1">
                                    {(course?.lessons || []).map((lesson, idx) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className={`p-1.5 rounded-lg ${isDone ? 'bg-green-500/20 text-green-400' : isActive ? 'bg-cyan-500 text-black' : 'bg-zinc-800 group-hover:bg-zinc-700'}`}>
                                                    {isDone
                                                        ? <CheckCircle2 size={14} />
                                                        : <Play size={14} fill={isActive ? 'currentColor' : 'none'} />}
                                                </div>
                                                <div className="flex-1 text-left">
                                                    <p className={`text-sm font-medium line-clamp-1 ${isDone ? 'text-green-400/80' : ''}`}>{lesson.title}</p>
